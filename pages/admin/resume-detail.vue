...
export default {
	...
	methods: {
		...
		async loadDetail() {
			showLoading('加载中...')
			try {
				// 通过 getCompanyResume 获取列表，然后找到对应记录
		...
		async updateStatus(status) {
			...
			try {
				...
				if (res && res.success) {
					this.resume.interviewStatus = status
					this.resume.readStatus = 1
					showToast('更新成功')
				} else {
		...
		async markRead() {
			try {
				await callCloud('updateResumeStatus', {
					deliverId: this.deliverId,
					readStatus: 1
				})
				if (this.resume) this.resume.readStatus = 1
			} catch (e) {
				// ignore
			}
		},
		...
		callPhone() {
			if (this.resume && this.resume.resumeInfo.phone) {
				makePhoneCall(this.resume.resumeInfo.phone)
			}
		},

		previewImage(idx) {
			if (this.resume && this.resume.resumeInfo.resumeFile) {
				previewImg(this.resume.resumeInfo.resumeFile, idx)
			}
		},
...
.action-section {
	...
	border-radius: 12rpx;
	padding: 24rpx;
	box-shadow: 0 2rpx 12rpx rgba(100, 170, 255, 0.1);
}
...
.bottom-bar {
	...
	box-shadow: 0 -2rpx 12rpx rgba(46, 199, 255, 0.15);
}
...
.btn-call {
	background: linear-gradient(135deg, #73C0FF, #23C343);
	color: rgba(10, 40, 140, 0.75);
}
